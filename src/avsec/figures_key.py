...
KEY_FIGURES: Tuple[Tuple[str, str, str], ...] = (
    ("K01", "Що кожен метод справді дає",
     "захист і якість поруч: висока якість без автентифікації - не захист"),
    ...
    ("K04", "Розміщення: компроміс без переможця",
     "пошкодження кодового слова проти втрати обох описів"),
    ("K05", "Звідки береться перевага", "внесок кожного кроку від B4 до P"),
...
def k01(ctx: FigureContext) -> Dict[str, Any]:
    ...
    fig.suptitle("K01. Що кожен метод справді дає", y=1.02, fontsize=12)
    footnote(ctx, fig,
...
def k10(ctx: FigureContext) -> Dict[str, Any]:
    ...
    if brute:
        ...
        a2.set_title(f"Перебір: {float(brute[0]['seconds']):.1f} с на {width} біт",
                     fontsize=10)
    fig.suptitle("K10. Схема 2021 року зламана", y=1.02, fontsize=12)
    footnote(ctx, fig,
