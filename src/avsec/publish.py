...
def primary_section(view: RunView) -> str:
    ...
    if better:
        lines.append(f"**Значуща перевага {a}** зафіксована в каналах: "
                     f"{', '.join(better)}.")
    if worse:
        lines.append(f"**Значуща перевага {b} над {a}** зафіксована в каналах: "
                     f"{', '.join(worse)}.")
    if flat:
...
def figures_section(view: RunView, figures_dir: str) -> str:
    ...
    return (f"Готово **{len(ready)} з {len(rows)}** рисунків каталогу.\n\n"
            "## Основні рисунки\n\n"
            "Вісім рисунків основного викладу. Кожен відповідає на окреме "
            "дослідницьке питання; решта каталогу — додаткові "
            "матеріали.\n\n"
            + _table(head, _body(main))
            + "\n\n## Додаткові матеріали\n\n"
              "Повний каталог: по одному рисунку на дослідницьке питання. "
              "Додаткові рисунки містять деталізацію ресурсного бюджету "
              "та перевірок моделі; вони призначені для контролю "
              "результатів.\n\n"
...
DOC_NAV = {
    "results": (
        "Усі таблиці дослідження, згенеровані з ОДНОГО прогону. Файл "
        "переписується командою `python scripts/publish.py`; ручне "
        "редагування буде втрачено під час наступної публікації.",
        "читач, який шукає конкретне число",
    ...
    "figures": (
        ...
        "читач, який шукає потрібний рисунок",
    ...
    "programme": (
        ...
        "читач, який перевіряє повноту роботи",
...
def doc_nav(kind: str) -> str:
    ...
    return ("<!-- DOCNAV -->\n"
            f"> **Призначення документа.** {what}\n>\n"
            f"> **Цільовий читач.** {who}.\n>\n"
            f"> **Пов'язані документи:** {nav} · [README](../README.md)\n"
            "<!-- /DOCNAV -->\n\n")
