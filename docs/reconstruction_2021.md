...
>
> **Пов'язані документи:** [каталог усіх схем](methods.md) · [місце серед інших робіт](related_work.md) · [чому перестановки недостатньо](threat_model.md) · [реєстр тверджень](claims.md) · [таблиці результатів](results.md) · [README](../README.md)
<!-- /DOCNAV -->
...
Будову, параметри й повні результати кожної схеми, згаданої нижче, зібрано
окремо — [methods.md](methods.md).
...
> [!NOTE]
> Кожна схема цієї частини описана повністю — будова, параметри, безпека
> й результати в усіх пʼяти профілях каналу — у каталозі
> [methods.md](methods.md). Тут наведено лише те, що кожен крок змінює
> відносно попереднього, і скільки це коштує.
...
```mermaid
flowchart LR
    subgraph TX["Передавальна сторона"]
        F["Кадр"] --> ST["Поділ на смуги<br/>по 24 рядки"]
        ST --> SRC["Кодування джерела DCT<br/>2 описи (MDC)"]
        SRC --> U["Сегмент → одиниця<br/>+ заголовок 56 Б"]
        U --> AEAD["AEAD<br/>ChaCha20-Poly1305"]
        AEAD --> FEC["FEC<br/>Рід — Соломон"]
        FEC --> PL["Розміщення BAWP<br/>у растрі"]
        PL --> MOD["Модуляція у растр"]
    end
    MOD --> CH(["Композитний відеосигнал<br/>наявний аналоговий тракт"])
    subgraph RX["Приймальна сторона"]
        CH --> SY["Пошук синхронізації"]
        SY --> GA["Оцінка підсилення<br/>і зміщення"]
        GA --> DEM["Демодуляція<br/>+ стирання"]
        DEM --> DIL["Обернене перемежування"]
        DIL --> DFEC["FEC-декодування"]
        DFEC --> HDR["Суворий розбір<br/>заголовка"]
        HDR --> VER["Перевірка AEAD<br/>+ свіжості"]
        VER --> ASM["Збирання<br/>+ карта доступності"]
    end
    ...
    style PL fill:#a5d6a7,stroke:#1b5e20
```
...
```mermaid
flowchart LR
    ST["Смуга кадру<br/>24 рядки"] --> D0["Опис 0<br/>стовпці 0, 2, 4, 6, …"]
    ST --> D1["Опис 1<br/>стовпці 1, 3, 5, 7, …"]
    D0 --> R0["Сам відновлює<br/>грубу версію смуги"]
    D1 --> R1["Сам відновлює<br/>грубу версію смуги"]
    R0 --> FULL["Два описи разом:<br/>повна вибірка"]
    R1 --> FULL
...
```mermaid
flowchart TD
    W["Крок 1. Растр ділиться на вікна<br/>по W рядків символів"]
    W --> G["Вікно ділиться на смуги<br/>G = max(1, min(D, W / B))"]
    G --> PAR["У профілі P:<br/>W = увесь растр, D = 2, B = 8,<br/>переливання дозволене"]
    PAR --> A["Крок 2. Одиниця опису d<br/>йде у смугу d mod G"]
    A --> SPILL{"Смуга<br/>заповнена?"}
    SPILL -->|"ні"| PLACE["Крок 3. Символ пишеться<br/>у клітинку смуги за правилом<br/>рядок і стовпець"]
    SPILL -->|"так"| OVER["Переливання<br/>в наступну вільну смугу"]
    OVER --> PLACE
    OVER -.->|"тут ламається<br/>ізоляція описів"| BREAK["Смуга несе два класи:<br/>пакет може зачепити три"]
    style PLACE fill:#c8e6c9,stroke:#2e7d32
    style BREAK fill:#ffcdd2,stroke:#b71c1c,stroke-width:2px
    style OVER fill:#fff9c4,stroke:#f9a825
```

Точне правило розкладання символів усередині смуги:

```text
   смуга має рядки [r0, r0 + h) і C стовпців;
   l-й символ, доданий до смуги, пишеться у клітинку:

      рядок    = r0 + (l mod h)
      стовпець = ( ⌊l / h⌋ + σ · (l mod h) ) mod C

   σ — публічна стала (column twist), у профілі P дорівнює 1
```
