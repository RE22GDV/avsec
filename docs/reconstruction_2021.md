...
<!-- DOCNAV -->
> **Що це.** Повна відповідь на завдання одним документом: відтворити схему захисту відеоканалу БпЛА зі статті ISITIA 2021, повторити її на справжніх знімках, показати межі й запропонувати вдосконалення.
>
> **Кому.** Тому, хто читає це **окремо від решти репозиторію** — документ самодостатній: кожна цифра тут супроводжується прогоном, комітом і командою, якою її можна перерахувати.
>
> **Поруч:** [місце серед інших робіт](related_work.md) · [чому перестановки недостатньо](threat_model.md) · [реєстр тверджень](claims.md) · [таблиці результатів](results.md) · [README](../README.md)
<!-- /DOCNAV -->
...
- [0. Як читати цей документ](#0-як-читати-цей-документ)
  - [Одна команда](#одна-команда)
  - [Звідки взяті числа](#звідки-взяті-числа)
- [Частина 1. Відтворення](#частина-1-відтворення)
  - [1.1 Що стаття справді повідомляє](#11-що-стаття-справді-повідомляє)
  - [1.2 Чого стаття **не** повідомляє](#12-чого-стаття-не-повідомляє)
  - [1.3 Як влаштований B1](#13-як-влаштований-b1)
  ...
  - [1.6 Про 65535 і про 2,2 с](#16-про-65535-і-про-22-с)
- [Частина 2. Повтор на різних фото](#частина-2-повтор-на-різних-фото)
  - [2.1 Звідки знімки](#21-звідки-знімки)
  - [2.2 Тріо для кожної сцени](#22-тріо-для-кожної-сцени)
  - [2.3 Результат залежить від вмісту, а не лише від seed](#23-результат-залежить-від-вмісту-а-не-лише-від-seed)
- [Частина 3. Атаки](#частина-3-атаки)
  ...
  - [3.5 Перебір seed](#35-перебір-seed)
  - [3.6 Підсумок: що саме зламано, а що ні](#36-підсумок-що-саме-зламано-а-що-ні)
- [Частина 4. Вдосконалення](#частина-4-вдосконалення)
  ...
  - [4.5 B4t → P і чесна поразка](#45-b4t--p-і-чесна-поразка)
  - [4.6 Підсумкова таблиця «атака → відповідь → ціна»](#46-підсумкова-таблиця-атака--відповідь--ціна)
- [5. Що відтворено, а що ні](#5-що-відтворено-а-що-ні)
- [6. Точне відтворення](#6-точне-відтворення)
...
Завдання має чотири частини, і документ побудований точно за ними:
...
Правило, якого документ дотримується без винятків: **жодного вигаданого
числа**. Кожна цифра нижче має підпис із прогоном і комітом, а команда, якою
її перераховують, наведена поруч.

### Одна команда

Частини 1 і 2 повністю відтворюються одним запуском:
...
### 1.1 Що стаття справді повідомляє
...
Seed **44257** окремо надрукований у підписі до рис. 8 статті. До нього ми
повернемося в [3.5](#35-перебір-seed).

### 1.2 Чого стаття **не** повідомляє
...
знаходить seed за секунди ([3.5](#35-перебір-seed)). Якщо взяти ширший
...
### 2.3 Результат залежить від вмісту, а не лише від seed
...
| [3.5](#35-перебір-seed) | перебір seed | ні | **одна пара** | 1 | знання розрядності регістра |
...
<sub><b>Рис. 2.</b> Та сама атака, та сама сітка, той самий seed — на
синтетичному градієнті й на чотирьох вирізках справжнього знімка з БпЛА.
...
Сказати «схема зламана» на підставі цього рисунка не можна. Сказати «схема
зберігає конфіденційність, бо кореляція 5,7 %» — теж не можна: 94,94 %
правильних сусідств і 24,68 дБ на гладкому вмісті означають, що великі
структури кадру видно без жодного ключа.
...
1. **Від вмісту.** На синтетичному градієнті 94,94 %, на справжніх знімках при
   ...
   аерознімок помітно складніший. Це **уточнює** поширену гіпотезу, що «БпЛА
   передає гладкий вміст, тож атака б'є саме по цільовому застосуванню»:
...
генерацію ([4.1](#41-b1--b2-криптографічний-генератор-щокадрово)), і сам
прогін чесно додає: успіх цієї атаки **не** є доказом стійкості щокадрового
варіанта.
...
(коміт <code>3e5f488</code>) дає той самий seed за 5,13 с —
<a href="img/isitia_b1_vs_b2.csv">isitia_b1_vs_b2.csv</a>.</sub>

**Знайдене число — 44257.** Це **той самий seed, який автори надрукували в
підписі до рис. 8 своєї статті**. Одна відома пара кадрів і п'ять секунд
звичайного ноутбука повертають його точно.

Два застереження, без яких це число можна прочитати неправильно:

* **65535 — параметр нашої реконструкції, а не властивість статті.** Автори
  розрядності не називають. Ширший регістр зробить саме цей перебір
  непрактичним;
* **і це нічого не змінить.** Атаки [3.1](#31-обраний-кадр),
  [3.2](#32-відома-пара), [3.3](#33-збирання-за-сумісністю-меж) і
  [3.4](#34-багатокадрова-атака) не дивляться на генератор узагалі. Вони
  працюють із самою перестановкою. Збільшення простору ключів закриває один
  рядок цієї таблиці з п'яти.

### 3.6 Підсумок: що саме зламано, а що ні
...
| Перебір seed | ні | одна пара | **знайдено seed 44257** | 5,14 с |
...
Точні висновки, у порядку сили твердження:

1. **Конфіденційності немає за жодного сценарію з відомим відкритим текстом.**
   Один кадр — і потік читається повністю. Це вимірювання, не гіпотеза.
2. **Навіть без відкритого тексту схема не тримає.** Багатокадрова атака дає
   половину таблиці за самим лише повторенням перестановки.
3. **Із самого лише перехопленого кадру витікає структура, а не весь кадр.**
   Формулювання «схема зламана» тут було б перебільшенням, а «кадр
   захищений» — применшенням.
4. **Автентичності немає взагалі.** Жодна з атак її не перевіряла, бо
   перевіряти нічого: у схемі 2021 року немає ні мітки автентичності, ні
   лічильника свіжості. Підміна кадру приймається беззастережно — а для
   відеоканалу БпЛА це щонайменше настільки ж серйозно, як і читання.
...
> [!NOTE]
> Рядки цієї таблиці тягнуть у різні боки, і це навмисно. `psnr_full` — якість
> **тих передач, що вдалися**. B3 показує кадр рідше, але коли показує —
> цілий, тож його умовне середнє вище на 7,40 дБ. B4 показує **частіше**
> (доступність 0,879 проти 0,627), включно з кадрами, де частина одиниць
> втрачена. Порівнювати схеми лише за `psnr_full` означало б винагороджувати
> ту, що відмовляється від складних кадрів — саме тому доступність і
> працездатність публікуються окремо й ніколи не змішуються в один показник.
...
**Це треба сказати прямо: B4t обганяє P на 1,19 дБ, і це значуще.** Тобто
запропоновані механізми (MDC + BAWP) у цьому прогоні не окупають своєї
вартості: та сама зміна транспорту **без них** дає більше.

Розклад E13 показує, куди насправді пішов виграш:
...
І сильніше за саму різницю чисел — те, що **зворотний порядок кроків не
проходить узагалі**: якщо додавати MDC і BAWP до B4 **до** зменшення одиниці,
вони не вміщуються в бюджет растра. Тобто ці два механізми не є самостійним
доповненням до базової схеми — вони застосовні лише після тих самих змін
транспорту, які й дають увесь виміряний виграш.

**Чому це не скасовує решту ланцюжка.** Кроки 4.1–4.4 відповідали на атаки з
частини 3; крок 4.5 відповідає на фізику каналу й **програє**. Ці два висновки
не конфліктують:
...
* MDC і BAWP у цьому прогоні **не виправдані** — і сказати це прямо важливіше,
  ніж захистити власну пропозицію. У каналах `clean` і `mild` простіша B4
  взагалі краща за P на 3,45 дБ, і це теж опубліковано
...
### 4.6 Підсумкова таблиця «атака → відповідь → ціна»
...
```bash
python -m avsec.cli verify --run results/main
```
