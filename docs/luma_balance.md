...
- [8. Стійкість до спотворень](#8-стійкість-до-спотворень)
  ...
  - [8.8 Чому спотворення діють саме так](#88-чому-спотворення-діють-саме-так)
  - [8.9 Зведення](#89-зведення)
...
> `ed61a8d81b2a58c2`, коміт `7fd685c`, робоче дерево чисте. Середовище:
...
Повідомлення живе в панелі 4. Прогін <code>ed61a8d81b2a58c2</code>, коміт
<code>7fd685c</code>.</sub>
...
найближчим кодовим словом. Прогін <code>ed61a8d81b2a58c2</code>, коміт
<code>7fd685c</code>.</sub>
...
### 8.8 Чому спотворення діють саме так

Розділи 8.3–8.5 показують, **скільки** коштує кожне спотворення. Вони не
показують, **чому** характер відмови настільки різний: амплітудний шум псує
кадр поступово, дрейф — стрибком, а звуження смуги кольоровості знищує його
цілком уже за найменшого ядра. Три різні поведінки мають три різні причини, і
кожну з них можна не постулювати, а виміряти.

Основа пояснення одна, і вона геометрична. Обмеження на сталу яскравість є
**одним лінійним рівнянням** `0,299·R + 0,587·G + 0,114·B = L`, тож палітра
лежить у **площині** тривимірного простору RGB, а кодові слова є точками цієї
площини. Декодування за найближчим кодовим словом порівнює відстані саме до
цих точок. Звідси випливають три наслідки, перевірені нижче.

<p align="center">
  <img src="img/luma_failure_geometry.png" width="980" alt="Площина палітри, напрям похибки, дія згладжування">
</p>

<sub><b>Рис. 5.</b> Ліворуч — палітра в координатах власної площини з
позначеними 256 кодовими словами монохромного шляху (показано проріджену
вибірку точок палітри). Посередині — та сама за величиною похибка, спрямована
в площину й поперек неї. Праворуч — дія згладжування окремо на однорідних і
неоднорідних вікнах. Прогін <code>ed61a8d81b2a58c2</code>, коміт
<code>7fd685c</code>.</sub>

#### 8.8.1 Напрям похибки важливіший за її величину

Довільний вектор похибки `e` розкладається на складову вздовж нормалі до
площини палітри й складову в самій площині. Перша виводить точку з площини, але
не змінює того, яка точка площини до неї найближча; друга рухає точку **вздовж**
розташування кодових слів і тому здатна підмінити найближче. Прогноз: за
однакової величини поперечна складова має коштувати істотно менше.

Щоб перевірити це чесно, обидві складові масштабовано до **однакового
середньоквадратичного значення**, тобто порівнюються напрямки, а не кількості.

| RMS похибки, рівнів | Ізотропна | У площині палітри | Поперек площини | PSNR поперечної |
| ---: | ---: | ---: | ---: | ---: |
| 0,5 | 99,63 % | 99,35 % | 100,00 % | точно |
| 1,0 | 98,54 % | 97,84 % | 99,90 % | 43,88 дБ |
| 2,0 | 93,80 % | 90,55 % | 99,26 % | 31,63 дБ |
| 4,0 | 76,11 % | 67,95 % | 97,80 % | 26,13 дБ |
| 8,0 | 44,05 % | **34,60 %** | **95,52 %** | 23,21 дБ |

<sub>Частка пікселів, відновлених точно, за декодування найближчим кодовим
словом. Джерело: <a href="img/luma_failure_geometry.csv">luma_failure_geometry.csv</a>.</sub>

Різниця виміряна й велика: за RMS 8 рівнів похибка в площині лишає 34,60 %
правильних пікселів, а така сама за величиною поперечна — 95,52 %, тобто майже
неушкоджений кадр. За RMS 0,5 поперечна похибка не псує **жодного** пікселя.
Ізотропний шум лежить між ними, бо на площину припадають лише дві третини його
енергії.

Звідси випливає й одна з причин, чому схема взагалі щось витримує: **третина
будь-якого ізотропного шуму безкоштовна**. Це не запас, закладений у схему, а
наслідок того, що повідомлення живе на двовимірній поверхні, а шум діє в трьох
вимірах.

Поперечна похибка все ж не є цілком безкоштовною: за RMS 8 вона лишає 95,52 %,
а не 100 %. Причина названа в розділі 4: у 8-бітній ґратці точно однакової
яскравості досягти неможливо, тож палітра лежить не в площині, а в **шарі**
завтовшки близько одного рівня — неокруглена яскравість кодових слів займає
проміжок [129,50; 130,49]. Достатньо велике поперечне зміщення взаємодіє з цією
товщиною.

#### 8.8.2 Згладжування не додає похибки, а підміняє слово

Амплітудний шум **рухає** точку; згладжування **змішує** сусідні точки. Це не
відмінність у силі, а відмінність у дії, і з геометрії випливає, чому друге
значно гірше.

Середнє двох точок площини є точкою тієї самої площини. Отже, результат
згладжування — не спотворене кодове слово, а **законна точка палітри**, яка
належить третьому, не спорідненому з обома, значенню повідомлення. Декодер не
має жодної ознаки, що сталася помилка: він знаходить найближче слово й видає
його значення.

Звідси випливає точний прогноз: вижити може лише піксель, **усе вікно ядра
якого несло одне значення** — тоді усереднення нічого не змінює. Прогноз
перевірено окремим підрахунком обох груп.

| Ширина ядра | Частка пікселів з однорідним вікном | Відновлено в таких вікнах | Відновлено в решті | Разом у кадрі |
| --- | ---: | ---: | ---: | ---: |
| 2 відліки | 6,14 % | **100,00 %** | 0,55 % | 6,66 % |
| 3 відліки | 0,86 % | **100,00 %** | 0,72 % | 1,58 % |
| 4 відліки | 0,21 % | **100,00 %** | 0,69 % | 0,89 % |

<sub>Вікно визначено так, як його читає застосоване згладжування
(`numpy.convolve`, режим `same`). Джерело:
<a href="img/luma_failure_geometry.csv">luma_failure_geometry.csv</a>.</sub>

Прогноз справдився у строгій формі: в однорідних вікнах відновлюється
**100,00 %** пікселів за всіх трьох ядер, у решті — від 0,55 до 0,72 %, тобто
рівень випадкового влучання. Частка відновлених пікселів у кадрі дорівнює
частці однорідних вікон із точністю до цього залишку: 6,14 % однорідних вікон
за ядра 2 відліки дають 6,66 % відновлення.

Це і є пояснення раптовості відмови з розділу 8.4. Ядро завширшки два відліки
не є слабким спотворенням: у природному кадрі сусідні пікселі майже завжди
різняться хоча б на одиницю, а **будь-яка** різниця значення означає цілком
інший колір, бо кодова книга не має порядку. Тому вже найменше ядро діє не на
частину пікселів, а майже на всі.

Тут же лежить і причина, чому зменшення глибини джерела не рятує від фільтра
(розділ 9.5), хоча рятує від шуму. Менше кодових слів стоять далі одне від
одного, і це допомагає, коли спотворення *переміщує* точку на фіксовану
відстань. Проти змішування таке розсування безсиле: середина відрізка між двома
словами лишається рівновіддаленою від обох, як би далеко вони не розійшлися,
бо зі збільшенням відстані обидві половини відрізка ростуть однаково.

#### 8.8.3 Одна помилка коштує стільки ж, скільки випадкове значення

Третій наслідок стосується не кількості помилок, а їхньої величини. Кодова
книга є перестановкою без властивості порядку: сусідні значення дістають
неспоріднені кольори, тож і навпаки — сусідні кольори належать неспорідненим
значенням. Хибно декодований піксель тому не «трохи неточний», а
**повністю втрачений**.

Еталоном є середнє `|v − u|` для `u`, взятого рівномірно з 256 значень,
обчислене за тими самими пікселями, що помилилися.

| σ | Хибних пікселів | Різних пар підміни | Середня похибка хибного пікселя | Еталон випадкового значення |
| ---: | ---: | ---: | ---: | ---: |
| 1 | 1,45 % | 53 | 55,11 | 71,73 |
| 2 | 6,28 % | 202 | 61,58 | 71,45 |
| 4 | 23,21 % | 660 | 67,04 | 71,22 |
| 8 | 55,40 % | 1584 | 69,97 | 71,06 |
| 16 | 82,32 % | 3601 | 71,49 | 71,06 |

<sub>Джерело: <a href="img/luma_failure_geometry.csv">luma_failure_geometry.csv</a>.</sub>

За σ = 16 середня похибка хибного пікселя (71,49) збігається з еталоном
(71,06): помилка нічим не відрізняється від випадкового вгадування значення.
За σ = 1 вона менша — 55,11 проти 71,73, — і стовпець «різних пар підміни»
показує чому. Усі помилки за цього рівня шуму складаються лише з 53 різних пар
значень, бо досяжні тільки геометрично близькі кодові слова. Це середнє за
малою вибіркою пар, а не інша поведінка: зі зростанням σ число пар зростає до
3601, і середнє сходиться до еталона.

Практичний висновок із цього пункту простий: **PSNR тут визначається кількістю
хибних пікселів, а не їхньою величиною**, бо величина стала. Саме тому крива
PSNR у розділі 8.3 повторює криву частки точних пікселів, а не йде окремо.

#### 8.8.4 Зведення механізмів

| Спотворення | Що робить геометрично | Чому відмова має саме такий характер | Чи рятує зменшення глибини |
| --- | --- | --- | :---: |
| Гауссів і рівномірний шум | випадково рухає точку, дві третини — у площині | поступова: частка хибних пікселів зростає плавно з σ | так |
| Дрейф підсилення й зміщення | рухає **всі** точки на той самий вектор | стрибок: доки зсув менший за половину відстані, помилок немає зовсім; далі помиляються всі | так |
| Звуження смуги кольоровості | замінює точку на середнє сусідніх, що є законною точкою палітри | повна: виживають лише однорідні вікна, а їх у кадрі одиниці відсотків | **ні** |
| Пакетне пошкодження | замінює цілі рядки довільними значеннями | пропорційна: решта кадру недоторкана | не стосується |

Остання колонка відрізняє два класи спотворень і тому має практичне значення:
зменшення глибини джерела збільшує відстань між кодовими словами, тож допомагає
проти всього, що **переміщує** точку, і не допомагає проти того, що **змішує**
сусідні слова. Ця межа підтверджена окремо на реальному тракті в розділі 9.5.
...
<sub><b>Рис. 6.</b> Стовпець 1 — вихідний кадр. Стовпець 2 — шифротекст зі
...
декодування за найближчим кодовим словом. Прогін <code>ed61a8d81b2a58c2</code>,
коміт <code>7fd685c</code>.</sub>
...
структури порядку, тож середнє двох її слів — це колір, який лежить далеко від
обох і майже ніколи не є кодовим словом. Гірше того, він лишається законною
точкою палітри: сталу яскравість задає лінійне рівняння, а середнє точок, що
його задовольняють, задовольняє його теж. Декодер тому не бачить жодної ознаки
помилки й видає значення третього, не спорідненого з обома, слова. Збільшення
відстані між словами тут не рятує: середина відрізка лишається рівновіддаленою
від обох кінців, хоч би як далеко вони розійшлися. Кількісно це показано в
[8.8.2](#882-згладжування-не-додає-похибки-а-підміняє-слово).
...
<sub><b>Рис. 7.</b> Ліворуч — залишкова похибка тракту проти межі декодування
для трьох глибин. Посередині — виміряний результат за глибиною джерела.
Праворуч — поруч зі схемами того самого бюджету. Прогін
<code>ed61a8d81b2a58c2</code>, коміт <code>7fd685c</code>.</sub>
...
**Що відтворено.** Схему, сформульовану в розділі 1, реалізовано й виміряно:
шифротекст має сталу яскравість у кожному пікселі, монохромний спостерігач
бачить рівне сіре поле, розшифрування для монохромного джерела точне до біта.
...
7. **Характер відмови визначає геометрія, а не сила спотворення.** Похибка
   поперек площини палітри коштує 95,52 % проти 34,60 % за похибки тієї самої
   величини в площині; згладжування ж підміняє кодове слово законним і тому
   лишає точними рівно ті пікселі, вікно яких несло одне значення
   (100,00 % проти 0,55 %). Це розділяє спотворення на такі, проти яких
   допомагає зменшення глибини джерела, і такі, проти яких не допомагає ніщо.

8. **На чистому каналі схема точніша за незахищене передавання.** `B2l`
...
словами, розклад механізму відмови за напрямом похибки, дією згладжування й
величиною однієї підміни, атаки на спотворений шифротекст, смуга зображень за
рівнями шуму, прогін через аналоговий тракт за пʼятьма профілями й шістьма
глибинами джерела, розклад профілю за спотвореннями й шість рисунків. Результат — у [`results/luma/`](../results/luma). Тривалість —
